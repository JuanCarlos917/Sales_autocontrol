...
const createVehicleWithPurchase = async (vehicleData, paymentData, userId) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (paymentData?.accountId && paymentData?.amount > 0) {
      ...
      // Actualizar estado del payable (contra mi deuda, no contra el precio total)
      const newPaidAmount = paymentAmount;
      const newStatus = newPaidAmount >= myOwedAmount ? 'PAID' : 'PARTIAL';
...
const confirmPurchase = async (vehicleId, vehicleData, paymentData, userId) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (paymentData?.accountId && paymentData?.amount > 0) {
      const paymentAmount = parseFloat(paymentData.amount);
      const account = await tx.account.findUnique({ where: { id: paymentData.accountId } });
      ...
      await tx.payablePayment.create({
        data: { payableId: payable.id, transactionId: transaction.id, amount: paymentAmount },
      });
